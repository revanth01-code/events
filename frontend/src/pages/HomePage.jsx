...
const HomePage = () => {
  ...
  useEffect(() => {
    if (location) {
      // Get nearby events and organizers
      const nearby = getNearbyEvents(location.lat, location.lng, 25);
      setFeaturedEvents(nearby.slice(0, 3));
      
      const organizers = getNearbyOrganizers(location.lat, location.lng, 25);
      setNearbyOrganizers(organizers.slice(0, 3));
    }
  }, [location]);
